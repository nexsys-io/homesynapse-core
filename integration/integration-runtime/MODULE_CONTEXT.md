...
| `ExceptionClassifier` | package-private | `classify(Throwable)` — the Doc 05 §3.7 table, M9.4a currency: PermanentIntegrationException→PERMANENT · InterruptedException→SHUTDOWN_SIGNAL · OOM/LinkageError→PERMANENT · **UnsupportedOperationException→PERMANENT (bare instanceof ONLY — an UnsupportedOperation is definitionally non-transient; a WRAPPED UOE stays TRANSIENT by design)** · **PIE-only cause-walk→PERMANENT (bounded getCause() walk, identity-hop cap 8 — cycle-safe; deliberately-thrown permanent intent survives wrapping; defense-in-depth — the checked seams should deliver PIE bare, the walk is the net, not the wire)** · everything else→TRANSIENT (the HA anti-pattern guard, unchanged). Shutdown-aware reclassification lives in the supervisor's run loop (per-adapter `shuttingDown` flag), not the pure classifier. Nothing maps to AUTH_FAILED yet (AMD-56 routing = deferred breadth). Pinned tests: wrapped-PIE→PERMANENT, bare-UOE→PERMANENT, wrapped-anything-else (incl. wrapped UOE)→TRANSIENT, cycle-bounded walk. `ExceptionClassification` PERMANENT javadoc reconciled (it already listed UOE — the code now matches). |
