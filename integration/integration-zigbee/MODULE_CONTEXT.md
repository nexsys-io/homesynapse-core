...
- **`identify` is not issuable through Tier-1 dispatch validation** (`StandardCommandValidator` requires a capability defining the command; none defines `identify`) — the hero gate drives the router+handler halves directly for that leg. Making identify end-user-issuable needs an identify capability or a validation carve-out — PM ruling queued (M9.4b candidate).
- **Brightness capability-domain mismatch (flagged):** ingestion normalizes `brightness` as the RAW 0–254 level ("percentage derives at query time") while the capability schema/derivation domain is 0–100 percent — a `set_brightness` confirmation would compare 72 (percent target) against e.g. 183 (level report) and false-timeout. The hero loop deliberately confirms on_off/CT only; the canonicalization ruling is queued (M9.4b candidate).
