...
- **`identify` is not issuable through Tier-1 dispatch validation** (`StandardCommandValidator` requires a capability defining the command; none defines `identify`) — the hero gate drives the router+handler halves directly for that leg. Making identify end-user-issuable needs an identify capability or a validation carve-out — the mechanism ruling is M9.4b work, **and the SEMANTICS are already PINNED (Nick, 2026-07-04 v18 beat 5, binding on M9.4b): an issuable-UNCONFIRMABLE command always renders the immediate `command_result("unconfirmed")` verdict with the recorded reason — never the silent DISABLED bypass ("never-tracked and honestly-verdicted are different promises"); the adapter verdict path is the enforcing seam, test-asserted at M9.4b.**
- **Brightness capability-domain mismatch — RULED (Nick, 2026-07-04 v18 beat 5): DERIVATION-side, superseding the earlier ingestion-side framing.** The mismatch: ingestion normalizes `brightness` as the RAW 0–254 level (Doc 08 §3.5 pins 0–254 as CANONICAL, "percentage derived at query time") while `StandardCapabilities.brightness()` declares a 0–100 schema — a `set_brightness` confirmation would compare a percent target against a level report and false-timeout. **The M9.4b fix shape (ruled): root-correct the `StandardCapabilities.brightness()` attribute schema 0–100 → 0–254 (it contradicts the Locked canonical), which makes the ledger's derivation bridge rescale GENERIC (param-domain → attribute-domain linear rescale when bounds differ — schema-driven, protocol-agnostic); §392's worked example already specifies `WithinTolerance` in LEVEL units (±2 of 128); PLUS query-time percent derivation lands before any dashboard brightness renders.** The hero loop deliberately confirms on_off/CT only until then.
- **The rig's callback pump is the keepalive nop:** unsolicited frames reach the protocol only while it reads (single-in-flight pipeline) — the rig queues callbacks and `deliverAndCycle()` pumps them through a `ping()` whose scripted response carries them (the `EzspInterviewTest` §G pattern). The production inbound pump binds with the real transport cadence (M9.4b).
