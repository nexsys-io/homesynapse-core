...
**Not fired** — the why-not card for NEVER_TRIGGERED with no run. Title `whyNot.headline.neverTriggered` "It hasn't run yet." (`whyNot.neverTriggered.title`, its unconsumed twin, RETIRED — FE-115 D3, the hub's ruling.) Body `whyNot.neverTriggered.body` "Nothing has set it off since this automation was loaded. It runs on {triggerSummary}. Nothing is wrong — it's waiting." (HERO-0 said "Not since this automation was loaded"; the full sentence reads aloud without the title.) The card keeps "What would make it run" with `triggerSummary` and, when the wire names one, "Watching: {entity}". No link is offered: there is no run to see. The scope "since it was loaded" is the honest scope until EXPLAIN-5 lands.
...
**Confirmation unknown** — an action step in two states. Settled UNCONFIRMED: label "Sent — the device never confirmed." and help "The command was sent; no confirmation came back{reasonClause}. It may have worked — the record can't say." (the HERO-1b keys `explain.action.unconfirmed.title` / `.body` were superseded by HERO-1c's `explain.mode.*` label / line / help rows and RETIRED — FE-115 D3, the hub's ruling) (HERO-0's "The hub sent {command} to {device}" named the actor and the raw command; the action line already names both, so the help states the fact.) Unsettled DISPATCHED: label "Sent — waiting for the device to confirm." and its help (the keys `explain.action.pending.title` / `.body` likewise superseded by `explain.mode.heldDispatched.*` and RETIRED — FE-115 D3); a color-class command adds `explain.action.pending.color` (kept — `format.pendingHint` reads it). It appears on the run page (the step) and, as the why-not verdict ACTED_BUT_UNCONFIRMED, on the why-not card with the link into that run.

## §5 The confirmation states

The five modes the standing law keeps distinct, plus the three neighbors a chain also carries. Label and shape carry the distinction; color reinforces; every row is AA in both themes (§8). Glyphs are the existing `MODE_GLYPHS` paths (`verdicts.ts:298`), so the build adds no new shape.
...
| EXPLAIN-4 — LANDED (FE-115 D1, as a sentence UNDER the line) | condition step line | The rule "state_equals" was false. (the type — L2 shows it is the type) | The L1 line keeps the type (the wire's `expression` is still the type — the rule TEXT is not on the wire); UNDER the observed state the v1.1.5 `conditions[].definition` renders as ONE sentence from its own catalog rows: "It checks that Hallway Motion occupancy is "detected"." · "It checks that the time is after 18:00 and before 23:30." · "It checks that Sun elevation is below 0 and Living Room Lamp brightness is below 20." (`explain.condition.def.*`, `format.definitionSentence`); PRESENT-null reads "What this rule checked isn't on record for this run."; ABSENT (a pre-v1.1.5 hub) adds nothing | `conditions[].definition` (v1.1.5, EXPLAIN-114c) |
...
| EXPLAIN-6 — LANDED (FE-114 D3) | why-not headline N3 | It has run — most recently at 9:42 pm. (info, an inference — N3 keeps this for a NEVER_TRIGGERED with a run id) | It ran at 9:42 pm, and the record says the device confirmed it. (ok; `whyNot.headline.firedConfirmed`, the pill `whyNot.pill.firedConfirmed`) | `FIRED_CONFIRMED` verdict — consumed |
...
| EXPLAIN-8 — LANDED (FE-114 D2) | why-not body, DISABLED | Turn it on in your automation settings to let it run. When it was turned off isn't recorded. (kept for `disabledAt` null / absent) | Turned off 3 hr ago — repeated_failure. Turn it on in your automation settings to let it run. (`whyNot.body.disabled.at`; the wire carries a reason, not an origin — shown as recorded) | `disabledAt` + `disabledReason` — consumed |
| EXPLAIN-9 — LANDED (FE-114 D1) | action step L2 "Confirmed" detail | Confirmed by the device's own report. (no time; kept for `confirmedAt` null / absent) | Confirmed at 9:42 pm, 0.4s after it fired. (`explain.action.detail.confirmedAt`; the action carries no issuedAt, so the interval is from the trigger, never "after the command"; `settledAt` typed and validated, not rendered) | `confirmedAt` — consumed |
...
| ~~`whyNot.neverTriggered.title`~~ RETIRED (FE-115 D3, the hub's ruling — a twin of the consumed `whyNot.headline.neverTriggered`; unconsumed, grep-proven) | It hasn't run yet. | why-not card title | — | frag. | 4 |
...
| ~~`explain.action.unconfirmed.title`~~ RETIRED (FE-115 D3, the hub's ruling — superseded by `explain.mode.*`; unconsumed, grep-proven) | Sent — the device never confirmed. | action step, settled UNCONFIRMED | — | frag. | 5 |
| ~~`explain.action.unconfirmed.body`~~ RETIRED (FE-115 D3, the hub's ruling — superseded by `explain.mode.*`; unconsumed, grep-proven) | The command was sent; no confirmation came back{reasonClause}. It may have worked — the record can't say. | action step help, settled UNCONFIRMED | reasonClause = ' — {reason}' | '' | 2.0 | 8 |
| ~~`explain.action.pending.title`~~ RETIRED (FE-115 D3, the hub's ruling — superseded by `explain.mode.heldDispatched.*`; unconsumed, grep-proven) | Sent — waiting for the device to confirm. | action step, unsettled DISPATCHED | — | 4.0 | 7 |
| ~~`explain.action.pending.body`~~ RETIRED (FE-115 D3, the hub's ruling — superseded by `explain.mode.heldDispatched.*`; unconsumed, grep-proven) | Most devices confirm within a second or two. This updates when the device reports. | action step help, unsettled DISPATCHED | — | 4.8 | 8 |
...
| `explain.mode.unknownOutcome.help` | The device reported an outcome this dashboard does not recognize yet — shown as recorded. | action step help | — | 7.6 | 14 |
...
| ~~`explain.hub.autos.noRunsSinceLoad`~~ RETIRED (FE-115 D3, the hub's ruling — "since it was loaded" is not something the wire can vouch for — `lastRunId` null renders `explain.hub.autos.noRuns`; unconsumed, grep-proven) | Hasn't run since it was loaded | hub automation row (lastRunId null, honest scope) | — | -1.4 | 6 |
...
| `explain.action.detail.confirmedAt` | Confirmed at {time}, {delta} after it fired. | confirmed action step help / pill title, `confirmedAt` a value (EXPLAIN-9; {time} = clockTimeWithDate(confirmedAt); {delta} = confirmedAt − trigger.matchedAt as "0.4s" — the action carries no issuedAt, so the interval is from the trigger, never "after the command") | FE-114 |
| `explain.action.detail.confirmedAt.noDelta` | Confirmed at {time}. | the same, when the trigger instant does not parse or is later than the confirmation (no interval claimed) | FE-114 |
| `whyNot.body.disabled.at` | Turned off {when}{reason}. Turn it on in your automation settings to let it run. | why-not body, DISABLED with `disabledAt` a value (EXPLAIN-8; {when} = timeAgo(disabledAt); {reason} = " — {disabledReason}", shown as recorded, or ""); null / absent keep `whyNot.body.disabled` | FE-114 |
| `whyNot.headline.firedConfirmed` | It ran at {time}, and the record says the device confirmed it. | why-not L1, FIRED_CONFIRMED (EXPLAIN-6; {time} = lastEvaluation.at — the EVALUATION instant, so it sits in the run clause; "the record says" is the Q1 register) | FE-114 |
| `whyNot.headline.firedConfirmed.noTime` | It ran, and the record says the device confirmed it. | why-not L1, FIRED_CONFIRMED with `lastEvaluation` null | FE-114 |
| `whyNot.pill.firedConfirmed` | Ran and confirmed | why-not pill, FIRED_CONFIRMED (ok tone, the check glyph; the link is `whyNot.link.ranFine`, no body) | FE-114 |
| `explain.terminal.completed.nothing.pill` | Completed, nothing changed | the do-nothing run's status pill and step label (an object-property literal the widened lint reached) | FE-114 |
| `explain.action.ran.on` | Ran {command} on | the loud / unnamed action line's verb for a command with no plain verb (a template beside `${`) | FE-114 |
| `explain.action.ran.unrecorded.on` | Ran an unrecorded command on | the same, `command` null | FE-114 |
| `ui.signingIn` | Signing in… | Resource's auth-state label (the app's `ui.*` register) | FE-114 |
| `explain.run.back` | ← All runs | run page back link | FE-114 |
| `whyNot.pill.sentNothing` | Ran, but sent nothing | why-not pill, `noCommandsIssued` true | FE-114 |
| `whyNot.pill.didRun` | It did run | why-not pill, N3 | FE-114 |
| `explain.condition.def.frame` | It checks that {clause}. | condition step, the definition sentence under the observed state (v1.1.5 `definition` an object; `format.definitionSentence` — the ONE place a definition becomes words; `{clause}` is the recursive operand) | FE-115 |
| `explain.condition.def.notRecorded` | What this rule checked isn't on record for this run. | the same, `definition` PRESENT-null (the projection could not vouch — hash mismatch, automation gone, index out of range; no reason is guessed). ABSENT adds nothing | FE-115 |
| `explain.condition.def.StateCondition` | {entity} {attribute} is "{value}" | the StateCondition clause ({entity} = the selector — a single-entity one through the registry census, a group selector as recorded; {value} as recorded, never coerced) | FE-115 |
| `explain.condition.def.StateCondition.noValue` | {entity} {attribute} matches a value that isn't recorded | the same, `value` null (a shape the emitter never writes — totality) | FE-115 |
| `explain.condition.def.NumericCondition.between` | {entity} {attribute} is above {above} and below {below} | the NumericCondition clause, both bounds ({above} / {below} the wire's numbers, no unit invented) | FE-115 |
| `explain.condition.def.NumericCondition.above` | {entity} {attribute} is above {above} | the same, `below` null | FE-115 |
| `explain.condition.def.NumericCondition.below` | {entity} {attribute} is below {below} | the same, `above` null | FE-115 |
| `explain.condition.def.NumericCondition.unbounded` | {entity} {attribute} has no bounds recorded | the same, both null | FE-115 |
| `explain.condition.def.TimeCondition.between` | the time is after {after} and before {before} | the TimeCondition clause, both edges ({after} / {before} the wire's "HH:MM", no locale conversion) | FE-115 |
| `explain.condition.def.TimeCondition.after` | the time is after {after} | the same, `before` null | FE-115 |
| `explain.condition.def.TimeCondition.before` | the time is before {before} | the same, `after` null | FE-115 |
| `explain.condition.def.TimeCondition.unbounded` | the time window isn't recorded | the same, both null | FE-115 |
| `explain.condition.def.AndCondition.join` | " and " (the connective, with its spaces) | AndCondition: the operands' clauses joined, in definition order | FE-115 |
| `explain.condition.def.OrCondition.join` | " or " (the connective, with its spaces) | OrCondition: the operands' clauses joined, in definition order | FE-115 |
| `explain.condition.def.NotCondition` | it is not true that {clause} | NotCondition (one operand; several render as a plain list — `.list.join` — no and/or asserted) | FE-115 |
| `explain.condition.def.list.join` | ", " | the plain-list connective (a NotCondition the wire gave several operands) | FE-115 |
| `explain.condition.def.group` | ({clause}) | an and/or operand with several children inside another compound — parenthesized so the connectives stay unambiguous; a leaf or a negation is not grouped | FE-115 |
| `explain.condition.def.emptyGroup` | an empty group of rules | an and/or/not with ZERO operands, nested (the arity is the wire's) | FE-115 |
| `explain.condition.def.emptyGroup.sentence` | This rule is an empty group — it has nothing to check. | the same at the top level (the whole sentence) | FE-115 |
| `explain.condition.def.ZoneCondition` | This is a zone rule — this hub doesn't record its details yet. | ZoneCondition (Tier 2, no operands) at the top level | FE-115 |
| `explain.condition.def.ZoneCondition.clause` | a zone rule (details not recorded) | the same, nested | FE-115 |
| `explain.condition.def.unknown` | This is a rule of kind "{type}" — this dashboard can't describe it yet. | a `type` this build does not know, at the top level — the raw type shown as recorded (the open vocabulary; never a throw) | FE-115 |
| `explain.condition.def.unknown.clause` | a rule of kind "{type}" (not described yet) | the same, nested | FE-115 |
| `explain.condition.def.entity.unnamed` | an unnamed device | the {entity} slot when `selector` is null (a shape the emitter writes only for permits without a selector — totality) | FE-115 |
| `explain.action.phrase.turnedOn` | Turned on | actionPhrase's verb for `turn_on` (the loud dangling-target line and the unnamed-target line) — a return Literal the widened lint reached (D4) | FE-115 |
| `explain.action.phrase.turnedOff` | Turned off | the same, `turn_off` | FE-115 |
| `explain.action.phrase.dimmed` | Dimmed | the same, `dim` (one word — outside the lint's pattern; keyed with its siblings) | FE-115 |
| `explain.chain.empty` | This run finished without recording any steps — no conditions were checked and no commands were sent. | the genuinely-empty chain's note (format.ts EMPTY_CHAIN_NOTE reads this row — the FE-114 D8 twin-fold; format.ts is outside the lint's six files by law) | FE-115 |
...
*§7 amendment, FE-115 (2026-09-19): the twenty-eight rows tagged FE-115 — the v1.1.5 `conditions[].definition` sentences (EXPLAIN-4 landed as a sentence under the condition line; 24 rows, `format.definitionSentence`) and the four literals D4 keyed (the lint's reach: `ReturnStatement > Literal` and `ConditionalExpression > Literal` join the six-file rule); six HERO-1b rows RETIRED on the hub's rulings (struck through above — D3). No existing sentence changed; D1 ADDS sentences.*

*§7 amendment, FE-114 (2026-09-14): the thirteen rows tagged FE-114 — the v1.1.4 keys' sentences (EXPLAIN-6 · EXPLAIN-8 · EXPLAIN-9) and the six literals the widened lint (IR-12: two or more words, apostrophes, a capital beside `${`, object properties) reached, byte-identical to what the screen showed. ONE text change: `explain.mode.unknownOutcome.help` (:207) now reads "recognize" (IR-13 — US is the dialect of record; the row was edited in place). Two rows this table already carried are now consumed: `explain.terminal.interrupted` (the INTERRUPTED terminal line changed from "Interrupted." to its row) and `explain.terminal.noSteps` (a completed run with `actionCount` 0 and no rows — the era skeleton included, §4). The format.ts twins (IR-14 and five more: `not recorded`, the no-reading line, the cascade line, "An earlier automation", the null-name note, the unnamed target) read the catalog.*

## §8 Accessibility

Contrast was computed from the generated `tokens.css` values at `eabdbb1` (WCAG 2.x relative-luminance formula; the pill text is 11–12 px, so the 4.5:1 small-text bar applies everywhere). Every state pair the hero uses passes AA in both themes: dark — ok 7.96 on ok-bg / 9.07 on surface, warn 7.92 / 8.97, error 6.97 / 7.19, info 7.47 / 8.04, unknown 7.24 / 7.59, body text 15.14, secondary 6.98, muted 4.92, link 8.04; light — ok 4.76 / 5.40, warn 5.84 / 6.56, error 4.64 / 5.40, info 6.04 / 6.79, unknown 5.17 / 6.04, body 17.46, secondary 6.79, muted 4.55, link 4.76. Three pairs the hero must avoid because they fall under 4.5: muted text on the sunk surface (3.96 dark, 3.95 light), a link on the sunk surface in light (4.12), and muted text on the page background in light (4.28). The L2 detail body therefore uses secondary text (5.62 / 5.89 on sunk), and any link inside an L2 body uses body text with an underline rather than the link color. The light ok and error pills sit at 4.76 and 4.64 — passing, with little margin; §9 asks for one token so a future darkening is a token change, not a component change.

Shape and label on every state: the marker glyph (check, arrow, clock, swap, ack-dots, x, arc, skip, dotted) and the label text carry the meaning; color and the dashed provisional outline reinforce. Each marker is `aria-hidden` and the step carries `explain.a11y.step` as visually-hidden text ("Step 2 of 4: action — Confirmed."); a provisional pill appends `explain.a11y.provisional`. The chain is a semantic `<ol>` with `explain.a11y.chain` as its label, never a tree widget. Focus order follows reading order: headline → each step's `<summary>` → the cascade link → the permanence footer; the `<summary>` elements are the only stops inside the chain, each with the visible blue focus ring (`--hs-focus-ring`), and a pill is never a focus stop (its help is the pill's `title` and is repeated as text where it matters — §5's help column — so nothing is hover-only). Under `prefers-reduced-motion: reduce` the settle transition has no animation: the pill swaps state without fade, the connector rail does not draw. Sentences stay ≤ 21 words (§7 measures each), which is itself the cognitive-accessibility half of the plain-language rule.
