...
- **D-01** — *DomainEvent non-sealed*: `WsSubscriptionFilter.eventTypes` is a list of `@EventType` strings, not a list of payload classes; registry lookup does the dispatch
- **D-04** — *Clock must be injected*: heartbeat, idle-timeout, and ws-session TTL logic takes `Clock` via constructor
