...
- **D-01** — *DomainEvent non-sealed*: WS subscription dispatch uses `@EventType` string matching against `WsSubscriptionFilter.eventTypes` set
- **D-05** — *`@EventType` on every event record*: event type strings drive WS subscription filtering
