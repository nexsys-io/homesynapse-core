...
**GOTCHA — `mint()`/`rotate()` advance the in-memory map BEFORE `persist()`** (pre-existing ordering, unchanged). A persist failure propagates as `UncheckedIOException` with the DISK intact but the map already advanced; the file governs at the next boot. The packaged readiness probe reads `initial_api_token` (`health-probe.sh:25`, `:85–:99`): a bare `revoke` of that key takes the service down at the next restart (403 → exit 3 → unit failed) and deleting the artifact makes the next restart wait out the 90 s probe timeout — both documented in `distribution/docs/token-rotation.md` (escalation E3 class).
