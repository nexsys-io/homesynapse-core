# rest-api — `com.homesynapse.api.rest` — Phase 3 transition (M3.6e.1) — HTTP command interface, RFC 9457 errors, 4-phase command lifecycle, idempotency keys, ReadinessFilter (Javalin before-handler)
...
| Type | Kind | Purpose | Key Details |
|---|---|---|---|
| `RestFilters` | **public** final utility class (no state) | Public gateway for installing REST-layer request filters onto the embedded HTTP server. The actual filter implementations are **package-private** per DEC-M3-16 — their Javalin-specific signatures must not appear in the rest-api module's exported public API because `requires io.javalin` is **non-transitive** (re-exporting Javalin via `requires transitive` would chain it into websocket-api and homesynapse-app, where it has no business). | Private constructor (utility class). Method: `static void installReadinessGate(Object javalinApp, ReadinessSource readinessSource)`. The `javalinApp` parameter is typed as `Object` (NOT `Javalin`) so the method signature does not leak `io.javalin.Javalin` to consumers; the method casts internally. The composition root in the lifecycle module already declares `requires io.javalin` independently, making the cast safe at the call site. |
...
**Total: 28 public types + 1 package-private + 1 module-info.java = 30 Java files.** (M3.6e.1 added the public `RestFilters` utility and the package-private `ReadinessFilter` — the first production handler in this module.)
