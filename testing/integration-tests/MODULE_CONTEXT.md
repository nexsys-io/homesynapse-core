...
| `EndpointE2eIT` | test class | E2E tests against the M3.6e.2 endpoints with real Jetty + real `HttpClient` + `json-unit-assertj` (REC-17). | Five `@Test` methods covering: empty listing, populated listing, single-entity 200, unknown-entity 404 with RFC 9457 problem detail, and the `/internal/dlq` response shape. **M7.5c-a:** the DLQ test asserts the frozen v1.1.1 §A5 envelope — `$.data.depth` (number), `$.data.parkedSubscribers` (array), `$.data.subscribers[*].subscriberId` contains the runtime subscribers (position-independent — the bus does not guarantee order), and `$.meta.viewPosition` (the freeze §0 poll-cursor pin at E2E level). Uses `Clock.fixed(...)` per DEC-M3-09. |
| `CrashRecoveryHttpIT` | test class (REC-19; M4.0a deliverable 8) | Crash-recovery scenario at the HTTP layer — exercises the full rehydration path end-to-end (M3.7 fix 2026-05-27; extended M4.0a 2026-05-29). | Two `@Test` methods. **(1) `entitiesSurviveCrashAndRestart`** (TESTING N=1 policy): every `onEvent()` flushes the coupled checkpoint; on restart the fresh `SqliteStateStore` rehydrates from the blob while the coupled subscriber checkpoint causes the bus to skip replay — recovery via the blob. **(2) `busAheadOfViewWindowClosedByReplayFromZero` (M4.0a, AMD-45 §3)**: uses `FixedCheckpointPolicy.HOME_DEFAULT` (200/2 s) + a frozen `Clock.fixed` + 5 (< 200) entities so NO checkpoint fires before `abandon()` — recovery MUST be via replay-from-zero (the bus subscriber checkpoint is 0, so it replays the durable log and the projection rebuilds all 5). Asserts `GET /api/v1/entities` returns 5 after restart. Before AMD-45 this fails (per-delivery bus checkpoint reaches 5 → replay skipped → state lost); after AMD-45 it passes. Both use `@TempDir(cleanup = CleanupMode.NEVER)` and route the crash through `HomeSynapseE2eHarness.abandon()` (the production-grade path — NOT the flag-based `IntegrationTestHarness.abandon()`; do not unify). |
