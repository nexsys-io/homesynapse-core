 ...
 */
@DisplayName("BusAwaitDiagnostic — the timeout text names the head, the awaited position and every subscriber's mode/checkpoint/dlq/behind (FIX-2a A)")
final class BusAwaitDiagnosticTest {
    ...
    @Test
    @DisplayName("render: one header line + one line per snapshot in list order, \\n-joined, no trailing newline; awaited prints as the position or 'none'")
    void render_headerThenOneLinePerSnapshot_exactText() {
        // Arrange — three snapshots: one behind the head by 2 (with a parked
        // entry), the atomic-checkpoint projection resting at 0, one LIVE at head.
        List<SubscriberSnapshot> snapshots = List.of(
                new SubscriberSnapshot("automation_engine", SubscriberMode.LIVE, 40L, 1, 0,
                        Instant.parse("2026-01-01T00:00:00Z")),
                new SubscriberSnapshot("state_projection", SubscriberMode.REPLAY, 0L, 0, 0, null),
