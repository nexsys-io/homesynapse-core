...
/**
 * Tests for {link Battery} — standard capability record.
 */
@DisplayName("Battery")
class BatteryTest {

    private static final ConfirmationPolicy POLICY = new ConfirmationPolicy(
            ConfirmationMode.DISABLED, List.of("battery_pct"), null, 5000L);

    private static Battery sample() {
        return new Battery("battery", 1, "core", Map.of(), Map.of(), POLICY);
    }
    ...
    class ConstructionTests {
        ...
        void allFieldsAccessible() {
            Battery cap = sample();
            assertThat(cap.capabilityId()).isEqualTo("battery");
        ...
        void exactlySixFields() {
            assertThat(Battery.class.getRecordComponents()).hasSize(6);
        }
        ...
        void isRecord() {
            assertThat(Battery.class.isRecord()).isTrue();
        }
    }
    ...
    class EqualsHashCodeTests {
        ...
        void differentNotEqual() {
            Battery a = sample();
            Battery b = new Battery("other", 1, "core", Map.of(), Map.of(), POLICY);
