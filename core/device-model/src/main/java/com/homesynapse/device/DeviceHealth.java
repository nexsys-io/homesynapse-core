...
/**
 * Standard read-only capability for device communication health metrics.
 *
 * <p>Provides two integer attributes: {@code rssi_dbm} (received signal strength
 * indicator in dBm) and {@code lqi} (link quality indicator, 0–255). These
 * metrics reflect the wireless link quality between the device and its
 * coordinator or parent node.</p>
 *
 * <p>No commands. Confirmation is {@link ConfirmationMode#DISABLED}.</p>
 *
 * <p>Capability ID: {@code "device_health"}. Defined in Doc 02 §3.6.</p>
 *
 * @param capabilityId the capability identifier, always {@code "device_health"} for standard instances
