...
- **GOTCHA (M3.5b update, 2026-05-18): The byte[0] stub is replaced by Jackson-serialized checkpoint data in the persistence module.** M3.5a wrote `byte[0]` from `StateProjection.writeCheckpoint`. M3.5b introduces `core/persistence`'s `CheckpointSerializer` + `SqliteStateStore.serialize(int projectionVersion)` so the projection can supply a real serialized snapshot — but the *projection* code itself was NOT modified in M3.5b (per scope brief). The wiring that has `StateProjection.writeCheckpoint` call `sqliteStateStore.serialize(projectionVersion)` instead of constructing `new byte[0]` is composition-root work tracked as a follow-up. M3.5b lands the infrastructure; the projection-side switch is the next milestone's concern.

- **GOTCHA (M3.5b, 2026-05-18): The authoritative `projectionVersion` for reconciliation now lives in the checkpoint data blob.** `SqliteStateStore.loadedProjectionVersion()` exposes the version embedded in the Jackson payload — this is the real version per AMD-41 §3.2.4. `CheckpointRecord.projectionVersion()` is still the M3.5a/M2.7 sentinel hardcoded to 1 by both `InMemoryViewCheckpointStore` and `SqliteViewCheckpointStore`. The reconciliation check in `StateProjection.initialize` currently reads from `record.projectionVersion()` — wiring it to consult `SqliteStateStore.loadedProjectionVersion()` instead is the next milestone's projection-side update.
...
*Added 2026-05-17 (Post-M3.1 refresh), revised 2026-05-18 (M3.5a + M3.5b complete). Phase 3 active — M3.5b State Projection Production Persistence landed 2026-05-18. Next milestone: M3.6 (StateQueryService implementation, ReadinessSource, lifecycle composition root). M3 governance: AMD-41/42/43 APPLIED. See `homesynapse-core-docs/design/HomeSynapse_Core_M3_Implementation_Plan_PLAN-M3-CONSOLIDATED-02.md` for the full M3 implementation plan.*

**M3.5b deliverables (2026-05-18) — landed in `core/persistence`, none in this module:**
...
**This module unchanged in M3.5b** beyond the two new gotchas above (byte[0] stub deprecation, projectionVersion authoritative source). `StateProjection` structural changes (consulting `SqliteStateStore.loadedProjectionVersion()` in reconciliation, calling `sqliteStateStore.serialize()` instead of constructing `new byte[0]`) are tracked as composition-root follow-up work.
