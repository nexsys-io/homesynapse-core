# state-store — `com.homesynapse.state` — 21 public + 5 package-private types — Materialized view over event stream, EntityState projection, availability tracking, checkpoint policy (sealed), bounded-window projection advancer, M3.5a vertical-slice StateProjection subscriber, M3.5b-wiring checkpoint-source injection seam, M3.6d-a readiness-source seam, M3.6e.1 MaterializedStateQueryService, M4.0b-1 production DerivationRule + DispatchingProjectionAdvancer (REC-28), M4.0b-2 version-transition reconciliation backfill (AMD-50); AMD-51 typed change-detection comparator RATIFIED 2026-05-30 (AMD-51-INV-01..05 registered below; production comparator + reconstruction = M4.0b-3, not yet started)
...
**Total: 21 public types + 5 package-private types (`SelfProducedFilter`, `MaterializedStateQueryService`, `ProductionDerivationRule`, `DispatchingProjectionAdvancer` + nested `ForwardingHandler`, `EnvelopeHandler`) + 1 module-info.java = 27 production Java files** (M4.0a added 1 public type — `AtomicCheckpointSink`; M3.6e.1 added 1 package-private type — `MaterializedStateQueryService` — and 1 public static factory method on `StateQueryService`; M4.0b-1 added 3 package-private types — `ProductionDerivationRule`, `DispatchingProjectionAdvancer`, `EnvelopeHandler` — and 2 public static factory methods, `DerivationRule.production()` and `ProjectionAdvancer.dispatching(EventStore)`).
...
- **GOTCHA (M4.0b-2, AMD-50): `lastChanged` has MIXED semantics — event-time when set by the reconciliation backfill, wall-clock when set by the LIVE `applyToState` `state_changed` branch.** This is a **conscious, named interim**, NOT a defect. M4.0b-2's new backfill write sources `lastChanged` from the causing event's log-fixed time (correct, rebuild-deterministic); the pre-existing LIVE `state_changed` branch still stamps `lastChanged`/`lastReported`/`lastUpdated` with the projection wall-clock (`clock.instant()`) across all branches — deliberately left untouched. Consequence: the full `EntityState` timestamp set is **not** rebuild-deterministic, so determinism/idempotency tests scope to attribute *values* + `stateVersion` (+ the backfill's event-time `lastChanged`), never byte-identical `EntityState`. The unifier (move the whole projection timestamp model to event-time) is a **separate scheduled WU** needing a design beat first (a behavioral-contract change to `EntityState`/Doc 03, likely AMD-11 staleness, with read-path blast radius into `MaterializedStateQueryService`) — do not silently expand into it.
