...
| `RunStatus` | enum (6 values) | Lifecycle state of an automation Run (§3.7) | `EVALUATING` (transient — evaluating conditions), `RUNNING` (transient — executing actions), `COMPLETED` (terminal — success), `FAILED` (terminal — error), `ABORTED` (terminal — cancelled), `CONDITION_NOT_MET` (terminal — does NOT consume mode slot) |
