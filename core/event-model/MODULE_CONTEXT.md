...
| `DomainEvent` | interface (marker) | Marker interface for all event payloads carried by EventEnvelope | Permanently non-sealed (AMD-33). Cannot be sealed because `IntegrationLifecycleEvent` in integration-api extends `DomainEvent` from a different JPMS module. All 18+ payload records implement this interface. |
