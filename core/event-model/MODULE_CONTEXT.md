# event-model — `com.homesynapse.event` — 47 types — Universal event vocabulary, EventEnvelope, EventPublisher, EventStore, all domain event payloads
...
| `DomainEvent` | interface (marker) | Marker interface for all event payloads carried by EventEnvelope | Permanently non-sealed (AMD-33). Cannot be sealed because `IntegrationLifecycleEvent` in integration-api extends `DomainEvent` from a different JPMS module. In event-model, 23 records implement this interface: 22 core payload records (all annotated with `@EventType`) plus `DegradedEvent` (the fallback wrapper, deliberately unannotated). |
...
**Total: 47 public types + 1 package-info.java + 1 module-info.java = 49 Java files.** (M2.1 added the `EventType` annotation.)
...
**GOTCHA: `DegradedEvent` is the ONLY `DomainEvent` implementor in event-model without `@EventType`.** All 22 core payload records carry `@EventType(EventTypes.CONSTANT)` as of M2.1. `DegradedEvent` is the fallback wrapper for events whose type is unknown or whose payload failed to upcast (Doc 01 §3.10) — it is never directly serialized under its own type discriminator but instead wraps an already-failed event. It must NOT be registered in `EventTypeRegistry`. The `EventTypeAnnotationTest.degradedEvent_doesNotHaveEventTypeAnnotation` test enforces this invariant. If you add a new `DomainEvent` payload record to this module, you MUST add `@EventType(EventTypes.YOUR_CONSTANT)` and extend `EventTypeAnnotationTest.EXPECTED_EVENT_RECORDS` plus bump `exactlyTwentyTwoAnnotatedRecords`'s expected count — the hardcoded list and count exist specifically to break loudly when the set changes.
...
|---|---|---|---|
| `EventStoreContractTest` | abstract class (27 `@Test` methods) | `com.homesynapse.event.test` | Defines the behavioral contract for `EventPublisher` + `EventStore`. Both `InMemoryEventStore` and the future `SqliteEventStore` must pass this suite. Subclasses implement three factory methods (`publisher()`, `store()`, `resetStore()`) and inherit the full test suite via JUnit. |
...
| `TestEventFactory` | final utility class (static factories + builders) | `com.homesynapse.event.test` | Static factory methods and builders for `EventDraft`, `EventEnvelope`, and `SubjectRef`. Public API: `draft()`, `draftFor(SubjectRef)`, `draftFor(SubjectRef, String)`, `draftBuilder()`, `envelope()`, `envelopeFor(SubjectRef)`, `envelopeBuilder()`, `subject()`, `deviceSubject()`, `integrationSubject()`, `automationSubject()`, `systemSubject()`, `personSubject()`. Contains public nested `Payload` record implementing `DomainEvent` for cross-module use, plus `DraftBuilder` and `EnvelopeBuilder` inner classes. |
...
- **`EventPublisher` needs a SQLite implementation:** `SqliteEventPublisher` in the persistence module. Must implement single-writer serialization, WAL commit, sequence assignment, global position assignment, and post-commit notification to EventBus. Must copy `EventDraft.actorRef()` to `EventEnvelope.actorRef()`. Must implement proper `eventType`→`EventCategory` mapping (InMemoryEventStore uses a placeholder).
- **`EventStore` needs a SQLite implementation:** `SqliteEventStore` in the persistence module. Must implement all 6 query methods with efficient SQLite queries and cursor-based pagination. See the EventStore query gotchas above — especially the `readBySubject` cursor semantics (`subjectSequence`, not `globalPosition`) and `readByTimeRange` effective time (`COALESCE(event_time, ingest_time)` with `[from, to)` range).
...
- **`EventTypeRegistry` — PLANNED (M2.4, persistence module).** Will scan classes annotated with `@EventType` at startup and build the `Map<String, Class<? extends DomainEvent>>` used by the Jackson-based deserializer to resolve the SQLite `eventType` discriminator column. Because JPMS scanning is fragile, M2.4 will most likely use explicit module-level registration (feeding the registry a hand-maintained list of record classes, one per module — event-model contributes its 22 records, integration-api contributes its 5 `IntegrationLifecycleEvent` subtypes once annotated). The registry reads `cls.getAnnotation(EventType.class).value()` for each registered class; any class missing the annotation is a registration-time error. `DegradedEvent` must never be registered — it is produced by the upcaster pipeline in lenient mode and is not a target of deserialization.
- **Integration lifecycle events need `@EventType` too (deferred).** The 5 `IntegrationLifecycleEvent` subtypes in `com.homesynapse.integration` (integration-api module) also implement `DomainEvent` and must receive `@EventType` before M2.4 can deserialize them. M2.1 deliberately scoped annotation application to event-model only. This is a separate mini-task in a different JPMS module.
- **Payload serialization:** Each DomainEvent record needs Jackson serialization support. The `EventSerializer` abstraction (from LTD-08) handles polymorphic deserialization using the `eventType` field as the type discriminator, with `EventTypeRegistry` (M2.4) providing the `String → Class` mapping sourced from the `@EventType` annotation applied in M2.1. Jackson's `@JsonTypeInfo` is banned by ArchUnit Rule 7 — the custom `@EventType` annotation is the approved alternative. Nullable `actorRef` (Ulid) serializes as `null` in JSON (LTD-08).
