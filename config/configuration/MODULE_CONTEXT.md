# configuration — `com.homesynapse.config` — 31 types — YAML 1.2 config loading, JSON Schema validation, AES-256-GCM secrets, hot reload
...
- **`com.homesynapse.config`** — All types in a single flat package. Contains: 3 enums (Severity, ReloadClassification, ChangeType), 11 data records (ConfigIssue, SecretEntry, ConfigMutation, ConfigSection, ConfigChange, MigrationChange, ConfigChangeSet, MigrationResult, MigrationPreview, ConfigModel, ReloadResult), 2 exceptions (ConfigurationLoadException, ConfigurationReloadException), 7 service interfaces (ConfigurationService, ConfigurationAccess, SecretStore, ConfigValidator, ConfigMigrator, SchemaRegistry, ConfigurationChangeListener — AMD-66, M6.1), 8 package-private implementation classes (ScopedConfigurationAccess — M6.1b; YamlLoader, JsonSchemaCompositeValidator, StandardSchemaRegistry, StandardConfigurationService — M6.1a; SchemaAnnotationIndex, ConfigModelDiff, AtomicYamlWriter — M6.4), and package-info.java.
...
| `YamlLoader` | final class (M6.1a) | Doc 06 §3.1 stages 1–2: safe-by-default YAML 1.2 parse + AMD-71 one-level `!include` splice with the canonicalization traversal guard | Ctor `(Path configDir)`. Explicit `CoreSchema` on BOTH root and included `LoadSettings` (⟳ gate-fix r1 — the engine default is the JSON schema; see the Core-schema gotcha). `load()` → nested `Result(Map<String,Object> document, List<ConfigIssue> issues)` — all issues FATAL; document empty when fatal. Layout constants `ROOT_DOCUMENT_NAME`/`INTEGRATIONS_DIRECTORY`. Includes parsed WITHOUT the `!include` constructor (nested include = unknown-tag FATAL, AMD-71-INV-02); containment via `toRealPath()` under `integrations/` (AMD-71-INV-01). Normalizes to string-keyed `LinkedHashMap`s; drops `null`-valued keys (null = unset → schema default; also satisfies the null-hostile `Map.copyOf` in the frozen records). Unknown tags (incl. `!secret`/`!env` until M6.2) are FATAL. |
...
**GOTCHA: `!secret` / `!env` tags are FATAL (unknown-tag) until M6.2.** M6.1a registers only the `!include` constructor; Doc 06 §3.1 stage-3 tag resolution arrives with the SecretStore (M6.2 — `!secret`) and should bring `!env` with it. A config using either tag today fails the load with a FATAL unknown-tag issue — safe-by-default, but means the stage-3 contract is partially staged. Zero-config and tag-free configs are unaffected.
