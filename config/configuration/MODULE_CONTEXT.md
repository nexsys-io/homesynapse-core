# configuration — `com.homesynapse.config` — 28 types — YAML 1.2 config loading, JSON Schema validation, AES-256-GCM secrets, hot reload
...
- **`com.homesynapse.config`** — All types in a single flat package. Contains: 3 enums (Severity, ReloadClassification, ChangeType), 11 data records (ConfigIssue, SecretEntry, ConfigMutation, ConfigSection, ConfigChange, MigrationChange, ConfigChangeSet, MigrationResult, MigrationPreview, ConfigModel, ReloadResult), 2 exceptions (ConfigurationLoadException, ConfigurationReloadException), 7 service interfaces (ConfigurationService, ConfigurationAccess, SecretStore, ConfigValidator, ConfigMigrator, SchemaRegistry, ConfigurationChangeListener — AMD-66, M6.1), 5 package-private implementation classes (ScopedConfigurationAccess — M6.1b; YamlLoader, JsonSchemaCompositeValidator, StandardSchemaRegistry, StandardConfigurationService — M6.1a), and package-info.java.
...
| `StandardConfigurationService` | final class implements `ConfigurationService` (M6.1a) | The M6.1 load path: parse → AMD-67 migrate → default-merge → validate → DP-2 error model → `ConfigModel` → AMD-70 publish | 10-arg ctor `(configDir, declaredSchemaMajor, declaredSchemaMinor, clock, systemId, eventPublisher, schemaRegistry, validator, migrators, listeners)`. AMD-66 §2.4: builds the listener map from the ruled `List` param; duplicate section path → ctor `IllegalArgumentException`. Migration: chain = `fromMajor ∈ [persistedMajor, declaredMajor)` ordered `(fromMajor, fromMinor)` (the `ConfigMigratorChainTest` reference semantics); applied in memory; document stamped to the declared pair; gap or newer persisted major → FATAL. `getCurrentModel()` volatile read, `IllegalStateException` pre-load. `reload()`/`write()` throw `UnsupportedOperationException` until M6.4. Publishes `config.validation_completed` once per completed validation pass (also on FATAL passes, before the throw); publish failure logged, never fails the load (AMD-70-INV-01). Writes the `schemas/config.schema.json` cache (failure = WARN, regenerable). |

**Total: 23 public types + 5 package-private classes + 1 module-info.java + 1 package-info.java = 30 Java files.** *(M6.1b: +`ConfigurationChangeListener` public, +`ScopedConfigurationAccess` package-private; `ConfigModel` 5→6 components, `ConfigMigrator` 3→5 methods, `MigrationPreview` 4→6 components per AMD-67. M6.1a: +4 package-private pipeline classes.)*
...
- **M6.1 status (2026-06-10):** **COMPLETE — both halves delivered, gates GREEN, COMMITTED.** M6.1b (access/events, AMD-66/67/70 shapes) committed `b7bc65c`. **M6.1a (load/validate, incl. ⟳ gate-fix round 1: CoreSchema both LoadSettings + ArchUnit Rule-5 config exemption per ratified AMD-71) committed `9035110`** — gate GREEN (27 + 147 tasks), PM WUCP Phase 2 APPROVE: `YamlLoader`, `JsonSchemaCompositeValidator`, `StandardSchemaRegistry`, `StandardConfigurationService` + `YamlLoaderTest`/`ConfigLayoutTest`/`JsonSchemaCompositeValidatorTest`/`StandardConfigurationServiceTest` (+`RecordingEventPublisher` test helper), the module-info/Gradle ruling applied, AMD-71 layout/include/traversal-guard behavior, the AMD-70 publish path, AMD-66 registration. No open deferred build gates.
- **ConfigurationService implementation DONE (M6.1a):** `StandardConfigurationService` — load path only. `load()`, `getCurrentModel()`, `getSection()`, AMD-66 §2.4 registration (ruled `List` ctor param), fires `config.validation_completed` (ruled metadata: DIAGNOSTIC, `EventOrigin.SYSTEM`, null eventTime, `publishRoot`, system subject, null actor). Takes the resolved config-dir `Path` (DP-3/AMD-71-A) + `Clock` (§4c) + `SystemId` + `EventPublisher` + registry/validator/migrators/listeners. **Reload (§3.3) and write (§3.5) are staged `UnsupportedOperationException` until M6.4** — M6.4 also brings: listener *invocation* + classification fallback, `config.section_reloaded` (+ its P2 survey re-run), `ConfigChangeSet` diffing, the reload/write `ReentrantLock`, atomic file writes, migration write-back + pre-migration backup (§3.7 step 7), and the per-ERROR `config_error` publication decision (flagged `[REVIEW]` at M6.1a).
